...
const updateProduct = async (req, res) => {
    try{
        ...
        let productData = { title, description, price, availableSizes }
        ...
        const updateedProduct = await productModel.findOneAndUpdate({_id}, {productData}, {new: true});
        return res.status(201).send({ status: true, message: "product data updated sucessfully", data: updateedProduct })
    }
    catch(err){
        res.status(500).send({ status: false, message: err.message });
    }
